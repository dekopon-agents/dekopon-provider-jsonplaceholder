...
The guest accepts only production JSONPlaceholder HTTPS or explicit literal loopback HTTP sockets. That validation is defense in depth, not authorization. Dekopon 0.11.1 remains authoritative for exact authority and method matching, DNS/destination controls, redirect behavior, one-call enforcement, deadlines, and byte limits. Production must keep `allowPlaintextLoopback: false`.

Read and create are distinct capabilities. Create remains a non-idempotent external write even though JSONPlaceholder returns a non-persistent synthetic record. A failure after POST may mean the effect executed.
