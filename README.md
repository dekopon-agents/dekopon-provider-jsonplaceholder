...
- `jsonplaceholder.posts.get`: low-risk, idempotent `read-only` GET of post IDs 1–100.
- `jsonplaceholder.posts.create`: medium-risk, non-idempotent `external-write` POST. JSONPlaceholder returns a synthetic record and does **not** persist it, but the call remains an external effect and may have executed even if response validation fails.
...
The checked-in WIT is caller-owned and byte-compared to the exact `dekopon-provider-sdk = 0.11.1` and `dekopon-provider-http = 0.11.1` crate contracts. The deterministic build requires Rust 1.97.0 (`rustc 1.97.0 (2d8144b78 2026-07-07)`) and `wasm-tools 1.236.1`.
...
The ignored output is `jsonplaceholder-provider.wasm` plus its `.sha256`. The decoded component exports the provider surface, imports exactly `dekopon:http/client@1.0.0`, and imports no WASI. Direct `dekopon-run` and an empty Wasmtime linker intentionally reject it; execution requires the broker.
...
./build.sh
./scripts/verify-component.sh
cargo +1.97.0 test --locked --test broker_host -- --nocapture
...
Version 0.1.0 is released only by the tag workflow after explicit human authorization. See [RELEASE.md](RELEASE.md). Actions creates exactly `jsonplaceholder-provider.wasm` and `jsonplaceholder-provider.wasm.sha256`, provenance and CycloneDX attestations, a non-latest GitHub release, and one `ghcr.io/dekopon-agents/provider-jsonplaceholder:0.1.0` OCI manifest with one `application/wasm` layer. Do not create a remote, tag, release, or package by hand.
